...
def ai_disposal(prod_name, lga_url):
    ...
    response = gpt.get_response(0.5,
        "First classify this product (" + prod_name + ") into a general category on the following website then use information from this site to tell me what I should do with it in 100 words:" + lga_url)
    return response
...
def ai_cf_wf(prod_name):
    ...
    response_carbon = gpt.get_response(0.2,
        "Please rate the sustainability of this product (" + prod_name + ") on a scale of 1 to 10, with 1 being not sustainable at all and 10 being extremely sustainable. Please take into consideration factors such as the materials used, the manufacturing process, the products lifespan, and its impact on the environment.")
